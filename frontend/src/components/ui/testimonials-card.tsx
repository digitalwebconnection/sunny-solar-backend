...
export function TestimonialsCard({
    ...
    return (
        <div className={cn("flex items-center justify-center p-", className)}>
            <div
                className="relative grid  grid-cols-[1fr]  md:grid-cols-[1fr_1fr] md:grid-rows-[auto_auto_auto] gap-x-2 gap-y-2 w-full "
                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className="absolute inset-0 w-full h-full overflow-hidden  bg-neutral-200 dark:bg-neutral-800 border-white dark:border-neutral-700 shadow-2xl rounded-lg"
                                    initial={{
                                        x: offset * 15,
                                        y: Math.abs(offset) * 6,
