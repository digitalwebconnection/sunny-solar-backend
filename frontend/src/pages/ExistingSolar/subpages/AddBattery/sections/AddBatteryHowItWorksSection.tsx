...
import { Sun, BatteryCharging, Moon, ShieldAlert, ArrowRight, CheckCircle, Zap, Activity, Cpu } from 'lucide-react';
...
export const AddBatteryHowItWorksSection: React.FC = () => {
  const steps = [
    ...
    },
    {
      ...
      icon: <Moon className="w-5 h-5 text-sky-600" />,
    ...
    },
    {
      ...
      icon: <BatteryCharging className="w-5 h-5 text-indigo-600" />,
  ...
  return (
    <section id="retrofit-flow" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Main Dynamic Layout: Visual Switchboard Showcase + Connected Energy Lifecycle */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch mb-10">
        
        {/* Left Side: Real Switchboard Installation Showcase (Normal Container) */}
        <div className="lg:col-span-5 bg-white border border-slate-200/90 shadow-sm overflow-hidden flex flex-col justify-between">
          <div className="relative min-h-[280px] sm:min-h-[340px] bg-slate-900 group">
            <img
            ...
            {/* Overlaid Badge */}
            <div className="absolute top-4 left-4 backdrop-blur-md bg-slate-900/85 px-3 py-1 rounded-md border border-white/20 text-white text-xs font-bold">
              Switchboard Connection
            </div>

            <div className="absolute bottom-4 left-4 right-4 text-white">
              <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider block">Zero Roof Intrusion</span>
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-200/80">
            <h3 className="text-sm font-bold text-slate-900 mb-2">Why AC-Coupling Beats Inverter Replacement</h3>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <div className="bg-white p-2.5 rounded-lg border border-slate-200 text-center">
                <span className="text-[10px] font-bold text-slate-400 uppercase block">Single & 3-Phase</span>
                <span className="text-xs font-extrabold text-slate-800">100% Compatible</span>
              </div>
              <div className="bg-white p-2.5 rounded-lg border border-slate-200 text-center">
                <span className="text-[10px] font-bold text-slate-400 uppercase block">Installation Time</span>
                <span className="text-xs font-extrabold text-emerald-600">Same-Day Active</span>
              </div>
            </div>
          </div>
        </div>

        {/* Right Side: 4 Connected Flow Stages (Non-Boxy Stepped Timeline Architecture) */}
